#Bootstrap to check the degree of modules
library(igraph)
BF <- read.table("PlotingNet/male_M2", quote="\"")
B_back <- read.table("~/Documents/MyLocal/NakedMole/FManalysi/SE_2v2/Background/BM_brain.back.gene", quote="\"")
Trans=c()
NPY_list2=c()
Edge_list2=c()
for(i in seq(1,100,1)){Sample_Set=toupper(c(as.character(BF$V1),as.character(B_back[sample(row.names(B_back),size=1),1])))
                     ...
                     Trans=c(Trans,length(E(Sample_g)))}
